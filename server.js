...
<body>
  ...
  <script>
    ...
    load();
    loadStats();
    setInterval(loadStats, 10000); // Оновлюємо статистику кожні 10 сек
  </script>
</body>
</html>`;
}
...
app.post('/v1/chat/completions', async (req, res) => {
  ...
  try {
    ...
    if (stream) {
      ...
      // Graceful shutdown — не рвемо стрімінг при перезапуску
    ...
    const message = err.response?.data?.detail ?? err.response?.data?.error?.message ?? err.message ?? 'Помилка';
