...
app.use(cors());
app.use(express.json({ limit: '10mb' })); // Для довгих чатів з великим контекстом
...
async function fetchWithRetry(axiosConfig) {
  ...
  while (true) {
    try {
      ...
      // 429 — читаємо Retry-After від NIM, якщо є
      if (status === 429 && attempts429 < config.max429Retries) {
        ...
        console.log(`[429] Retry ${attempts429}/${config.max429Retries} через ${delay}ms (Retry-After: ${retryAfter > 0 ? retryAfter + 'ms' : 'не вказано'})`);
        await new Promise(r => setTimeout(r, delay));
        continue;
      }

      // 5xx — фіксована пауза з подвоєнням
...
<body>
  ...
  <div class="card">
    ...
    <div class="row">
      <div><div class="lbl">Температура</div><div class="desc">Якщо клієнт не передав</div></div>
      <input class="num" type="number" id="defaultTemperature" min="0" max="2" step="0.05">
    </div>
    <div class="row">
      <div><div class="lbl">Макс. токени</div><div class="desc">Якщо клієнт не передав</div></div>
      <input class="num" type="number" id="defaultMaxTokens" min="256" step="256">
    </div>
    <div class="row">
      <div><div class="lbl">Таймаут (мс)</div><div class="desc">Для довгих відповідей</div></div>
  ...
  <script>
    ...
    async function load() {
      ...
      } catch { statusEl.innerHTML = '<span class="err">Не вдалося завантажити</span>'; }
    }
    ...
    async function save() {
      ...
      try {
        ...
        if (r.ok) {
          ...
          statusEl.innerHTML = '<span class="err">' + (e.error || 'Помилка') + '</span>';
        }
...
app.post('/v1/chat/completions', async (req, res) => {
  ...
  const rawKeyString = authHeader.startsWith('Bearer ') ? authHeader.slice(7).trim() : null;
  if (!rawKeyString) return res.status(401).json({ error: { message: 'Відсутній API ключ', code: 401 } });

  // Розбиваємо рядок на масив ключів (через кому) та беремо випадковий
  const apiKeys = rawKeyString.split(',').map(k => k.trim()).filter(Boolean);
  if (apiKeys.length === 0) return res.status(401).json({ error: { message: 'Невалідний API ключ', code: 401 } });
  ...
  // ─── ОЧИСТКА ПОВІДОМЛЕНЬ ДЛЯ JANITOR AI / MISTRAL ───
  ...
  for (const msg of messages) {
    ...
    if (cleanMsg.role === 'system') {
      ...
      if (lastMsg && lastMsg.role === cleanMsg.role) {
        lastMsg.content += "\n\n" + cleanMsg.content;
      } else {
  ...
  // Об'єднаний system промпт завжди на нульовому індексі
  if (systemContent) {
  ...
  try {
    ...
    if (stream) {
      ...
      const onClose = () => { response.data.destroy(); res.end(); };
      ...
      response.data.on('end', () => { process.off('SIGTERM', onClose); res.end(); });
      response.data.on('error', () => { process.off('SIGTERM', onClose); res.end(); });
    ...
    let message = 'Помилка';
    if (rawData?.detail) {
      message = typeof rawData.detail === 'string' ? rawData.detail : JSON.stringify(rawData.detail);
    ...
    console.error(`[${status}] Деталі NVIDIA API:`, JSON.stringify(rawData ?? err.message));
...
app.listen(PORT, () => console.log(`✅ Проксі на порту ${PORT} | /admin для налаштувань`));
